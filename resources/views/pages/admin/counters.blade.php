...
<div class="flex items-center justify-between">
    ...
    <button onclick="document.getElementById('modal-counter').style.display='flex'"
...
<form method="GET" action="{{ route('admin.counters') }}" class="mb-5 bg-white dark:bg-gray-900 rounded-2xl shadow-sm ring-1 ring-gray-200 dark:ring-gray-800 p-4 grid gap-3 md:grid-cols-3">
    <div>
        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Filtrer par service</label>
        <select name="service_id"
            ...
            <option value="{{ $service->id }}" {{ (string) $selectedServiceId === (string) $service->id ? 'selected' : '' }}>{{ $service->name }}</option>
            @endforeach
        </select>
    </div>
    <div>
    ...
    <div class="flex items-end gap-2">
        ...
        <a href="{{ route('admin.counters') }}" class="px-4 py-2.5 rounded-xl bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-sm font-semibold text-gray-700 dark:text-gray-300 transition">Réinitialiser</a>
    </div>
</form>
...
<div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-sm ring-1 ring-gray-200 dark:ring-gray-800 p-5 flex flex-col gap-4">
        <div class="flex items-start justify-between">
            ...
            <p class="text-[11px] text-gray-400">{{ $counter->created_at->format('d/m/Y H:i') }}</p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-2">
            <button type="button"
                onclick="document.getElementById('modal-edit-counter-{{ $counter->id }}').style.display='flex'"
                class="w-full py-2 rounded-xl bg-indigo-50 dark:bg-indigo-950 hover:bg-indigo-100 dark:hover:bg-indigo-900 text-indigo-700 dark:text-indigo-300 text-xs font-semibold transition">
                Modifier
            </button>
...
@foreach($counters as $counter)
<div id="modal-edit-counter-{{ $counter->id }}" style="display:none" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
    <div class="w-full max-w-md bg-white dark:bg-gray-900 rounded-3xl shadow-2xl p-7 ring-1 ring-gray-200 dark:ring-gray-700">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-lg font-black text-gray-900 dark:text-white">Modifier le guichet</h2>
            <button type="button" onclick="document.getElementById('modal-edit-counter-{{ $counter->id }}').style.display='none'"
        ...
        <form method="POST" action="{{ route('admin.counters.update', $counter) }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ $counter->name }}" required
            ...
            <div>
                ...
                <select name="service_id" required
                    ...
                    <option value="{{ $s->id }}" {{ $counter->service_id === $s->id ? 'selected' : '' }}>{{ $s->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Agent</label>
                <select name="agent_user_id"
                    class="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition cursor-pointer">
                    <option value="">-- Aucun agent --</option>
                    @foreach($agents as $a)
                    @if($a->counters_count === 0 || $counter->agent_user_id === $a->id)
                    <option value="{{ $a->id }}" {{ $counter->agent_user_id === $a->id ? 'selected' : '' }}>{{ $a->name }}</option>
                    @endif
                    @endforeach
                </select>
            </div>
...
{{-- Modal création --}}
<div id="modal-counter" style="display:none" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
    <div class="w-full max-w-md bg-white dark:bg-gray-900 rounded-3xl shadow-2xl p-7 ring-1 ring-gray-200 dark:ring-gray-700">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-lg font-black text-gray-900 dark:text-white">Nouveau guichet</h2>
            <button onclick="document.getElementById('modal-counter').style.display='none'"
                class="h-8 w-8 flex items-center justify-center rounded-full bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-500 transition text-lg">&times;</button>
        </div>
        <form method="POST" action="{{ route('admin.counters.store') }}" class="space-y-4">
            ...
            <div class="flex gap-3 pt-2">
                <button type="button" onclick="document.getElementById('modal-counter').style.display='none'"
                    class="flex-1 py-3 rounded-xl bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-sm font-semibold text-gray-700 dark:text-gray-300 transition">Annuler</button>
                <button type="submit"
